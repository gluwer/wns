...
// The mapping of currently valid client_secret:client_id to accessToken. If accessToken is not available or times out (401),
// there will be additional request made and value here will be updated.
// There is no automatic cleaning of old access tokens, so run cleanAccessTokenCache() function.
...
var releasePendingNotifications = function (context, error) {
    ...
    pendingContexts.forEach(function (item) {
      // release errors asynchronously
      process.nextTick(function(){
        onError(item, error);
      });
		});
	else
    async.eachLimit(pendingContexts, 5, function (item, next) {
			item.newAccessToken = context.newAccessToken;
      sendNotificationNow(item, next);
		}, function(){/* completion is not reported anywhere */});
}
...
var obtainAccessToken = function (context) {
  ...
  request.post({
    pool: false, // access token retrieval is rare, no need for connection pooling
    uri: 'https://login.live.com:443/accesstoken.srf',
    form: {
    ...
    if (err) {
      var result = new Error('Unable to send reqeust for access token to Windows Notification Service: ' + err.message);
      result.innerError = err;
      return releasePendingNotifications(context, result);
      ...
      if (!body || typeof body.access_token !== 'string' || body.token_type !== 'bearer') {
        ...
        error.innerError = new Error('Invalid response');;
        return releasePendingNotifications(context, error);
      } else {
        context.newAccessToken = body.access_token;
        accessTokenCache[key] = body.access_token;
        return releasePendingNotifications(context);
      }
...
var sendNotificationNow = function (context, cb) {
  var key = getClientKey(context);
	var accessToken = context.newAccessToken || context.options.accessToken || accessTokenCache[key];

	// construct the request to WNS

	var options = {
    forever: true, // it may be called many times, keep the socket
		uri: context.channel,
...
exports.send = function () {
	...
	if (options.accessToken || accessTokenCache[getClientKey(context)])
...
exports.clearAccessTokenCache = function() {
  accessTokenCache = {};
};
